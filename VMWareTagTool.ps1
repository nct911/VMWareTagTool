...
"@
        ...
        Function Get-VMData {
            <#
                ...
                .DESCRIPTION
                Retrieves VMs and their tags from the currently specified vCenter.
                ...
                .OUTPUTS
                VM Data is applied to form
            #>

            $WPFGui.UI.Dispatcher.invoke([action] { $WPFGui.VMList.Clear() })
            $LocalVMList = Get-VM | Sort-Object Name
            Write-Activity -Prefix "Data" -Stream 'Output' -Text "Found $($LocalVMList.Count) VM(s)"
            Write-StatusBar -Progress 33 -Text "Found $($LocalVMList.Count) VM(s)"
            Write-Activity -Prefix "Data" -Stream 'Output' -Text "Retrieving tag assignments..."
            $VMsWithTags = $LocalVMList | Select-Object @{Name = 'Key'; Expression = { $_.Name } }, @{Name = 'Value'; Expression = { (Get-TagAssignment -Entity $_ ).Tag.Name -join ', ' } }
            $WPFGui.UI.Dispatcher.invoke([action] {
                    $VMsWithTags.Foreach({
                            $WPFGui.VMList.Add($_)
                        })
            ...
            Write-Activity -Prefix "Data" -Stream 'Output' -Text "Getting Tags and Categories"
            Write-StatusBar -Progress 66 -Text "Retrieving tags and categories..."
            $WPFGui.UI.Dispatcher.invoke([action] {
                    $WPFGui.CategoryList.Clear()
                })
            $TagCategories = New-Object System.Collections.ArrayList
            Get-TagCategory | Foreach-Object {
                $TagCategories.Add( [PSCustomObject] @{
                        Name        = $_.Name
                        Description = $_.Description
                        Tags        = (Get-Tag -Category $_ | Sort-Object )
                    })
            }
            $WPFGui.UI.Dispatcher.invoke([action] {
                    $TagCategories.ForEach({ $WPFGui.CategoryList.Add($_) })
                })
        }
        $SessionFunctions.Add('Get-VMData') | Out-Null
        function Connect-VIServerVerbose {
            <#
                .SYNOPSIS
                Connect to VIServer with progress bar and activity log output

                .DESCRIPTION
                Connect to VIServer with progress bar and activity log output

                .PARAMETER VIServer
                The VIServer to connect.
                ...
                Connect-VIServerVerbose -VIServer Exaple-Host.local -Credential $MyCredentials

                .NOTES

                .INPUTS
                VIServer and Credential
            ...
            $Return = $false
            $Error.Clear()
            Write-Activity -Prefix "Connect" -Stream 'Output' -Text "Connecting to VIServer $VIServer"
            Write-StatusBar -Progress 50 -Text "Connecting to vCenter $VIServer"
            try {
                Connect-VIServer -Server $VIServer -Protocol https -Credential $Credential  -Verbose 
                Write-Activity -Prefix "Connect" -Stream 'Output' -Text "Connected"
                Write-StatusBar -Progress 100 -Text "Connected to vCenter $VIServer"
                $return = $true
            }
            catch {
                Write-Activity -Prefix "Error" -Stream 'Output' -IsError -Text ($Error | Out-String)
                Write-StatusBar -Progress 0 -Text 'Connecting Error'
            }
            return $return
        }
        $SessionFunctions.Add('Connect-VIServerVerbose') | Out-Null
        
        function Disconnect-VIServerVerbose {
            <#
                .SYNOPSIS
                Disconnect from VIServer with progress bar and activity log output
                ...
                Disconnect-VIServerVerbose -VIServer Exaple-Host.local -Credential $MyCredentials
            ...
            $Return = $false
            $Error.Clear()
            Write-Activity -Prefix "Disconnect" -Stream 'Output' -Text "Disconnecting from VIServer $VIServer"
            Write-StatusBar -Progress 50 -Text "Disconnecting from vCenter $VIServer"
            try {
                Disconnect-VIServer * -Force -Confirm:$false
                Write-Activity -Prefix "Disconnect" -Stream 'Output' -Text "Disconnected"
                Write-StatusBar -Progress 100 -Text "Disconnected from vCenter $VIServer"
                $return = $true
            }
            catch {
                Write-Activity -Prefix "Error" -Stream 'Output' -IsError -Text ($Error | Out-String)
        ...
        $SessionFunctions.Add('Disconnect-VIServerVerbose') | Out-Null
        function Update-TagAssignment {
            <#
                .SYNOPSIS
                Update Tag Assignments

                .DESCRIPTION
                Update Tag Assignments according to the user's selections

                .PARAMETER Assign
                Add assignments for the selected tag to the selected VMs

                .PARAMETER Unassign
                Remove assignments for the selected tag to the selected VMs
            ...
            #>
            param (
                # Prefix text, describe which parti s printing output
                [switch]
                $Assign,
                [switch]
                $Unassign
            ...
            #This just moves the if/then outside the asynchronous code. Mostly just to have it setup before running.
            if ( $Assign ) {
                $Verb = 'Assign'
                $Direction = 'to'
            }
            ...
            $AsyncRunParameters = @{
                Variables = @{
                    'WPFGui'         = $WPFGui
                    'VIServer'       = $WPFGui.VCName.Text
                    'Credential'     = $WPFGui.Credential
                    'TagName'        = $WPFGui.TagSelector.SelectedItem.Name
                    'TagCategory'    = $WPFGui.CategorySelector.SelectedValue
                    'SelectedVMList' = $WPFGui.VMGrid.SelectedItems.Key
                    'Direction'      = $Direction
                    'Verb'           = $Verb
                }
                Code      = {
                    $WPFGui.UI.Dispatcher.invoke([action] {
                            $WPFGui.ControlGroup.IsEnabled = $false
                        })
                    $Error.Clear()
                    Try {
                        if ( Connect-VIServerVerbose -VIServer $VIServer -Credential $Credential ) {
                            # Make ONE trip to the vCenter for the VMs, then iterate over the collection
                            $SelectedVMs = Get-VM $SelectedVMList
                            $Progress = 0
                            $ProgressStep = 100 / ($SelectedVMs.Count * 2)
                            if ( $Verb -eq 'Assign') {
                                $Tag = Get-Tag -Name $TagName -Category $TagCategory.Name
                            }
                            Foreach ( $VM in $SelectedVMs ) {
                                $Progress += $ProgressStep
                                Write-StatusBar -Progress $Progress -Text "$Verb tag $TagName $Direction $($VM.Name)"
                                Write-Activity -Prefix $Verb -Stream 'Output' -Text "$Verb`ing tag $TagName $Direction $($VM.Name)"
                                switch ($Verb) {
                                    'Assign' { 
                                    ...
                                    'Unassign' {
                                        $TagAssignment = Get-TagAssignment -Entity $VM -Category $TagCategory.Name | Where-Object { $_.Tag.Name -eq $TagName }
                                ...
                                Write-StatusBar -Progress $Progress -Text "Updating tag list for $($VM.Name)"
                                Write-Activity -Prefix $Verb -Stream 'Output' -Text "Updating tag list for $($VM.Name)"
                                $NewTags = (Get-TagAssignment -Entity $VM).Tag.Name -join ','
                                if ( -not $NewTags ) {
                                    $NewTags = " "
                                    Write-Activity -Prefix $Verb -Stream 'Output' -Text "No tags remain."
                                }
                                else {
                                    Write-Activity -Prefix $Verb -Stream 'Output' -Text $NewTags
                                }

                                # Update GUI to reflect changes
                                $WPFGui.UI.Dispatcher.invoke([action] {
                                        $WPFGui.VMList.Where( { $_.Key -eq $VM.Name })[0].Value = $NewTags
                                        $WPFGui.VMList.CollectionChanged
                                        $WPFGui.VMGrid.Items.Refresh()
                                        $WPFGui.VMGrid.UpdateLayout()
                                    })
                            }
                            Disconnect-VIServerVerbose
                            Write-StatusBar -Progress 0 -Text 'Ready'
                        }
                    }
                    catch {
                        Write-Activity -Prefix "Assign" -Stream 'Output' -Text ($Error | Out-String) -IsError
                    }
                    $WPFGui.UI.Dispatcher.invoke([action] {
                            $WPFGui.ControlGroup.IsEnabled = $true
                        })
                    Write-StatusBar -Progress 0 -Text "Ready"

                }
            }
            Invoke-Async @AsyncRunParameters
        }

        function Sync-UpdateVMsButton {
            <#
                .SYNOPSIS
                Synchronize UpdateVMs button to form state

                .DESCRIPTION
                Enable or Disable the UpdateVMs button depending on the state of the form

                .EXAMPLE

                Sync-UpdateVMsButton
            ...
            # Require a vCenter name and user credentials prior to allowing the user to try to retrieve VM details.
            if ($WPFGui.VCName.Text -and $WPFGui.VCUsername.Text -and ($WPFGui.VCPassword.SecurePassword.Length -gt 0) ) {
                $WPFGui.UpdateVMs.IsEnabled = $true
            }
            else {
                $WPFGui.UpdateVMs.IsEnabled = $false
            }
        }
        $SessionFunctions.Add('Update-TagAssignment') | Out-Null
            ...
            <DockPanel Name="MainDockPanel" Margin="5,5,5,2.5" Grid.Row="1" Grid.Column="1" >
                ...
                <GroupBox Name="ControlGroup" Padding="0" DockPanel.Dock="Left" IsEnabled="True">
                    <StackPanel DockPanel.Dock="Left" Orientation="Vertical" Width="180" Margin="5,5,5,5">
                        ...
                        <Button DockPanel.Dock="Top" Name="Assign" Padding="0" Margin="0,20,0,0" VerticalAlignment="Top" HorizontalContentAlignment="Stretch" Height="28" IsEnabled="False" >
                            <DockPanel  Margin="4,0" >
                                <Viewbox DockPanel.Dock="Left" xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" Height="22" Stretch="Uniform" HorizontalAlignment="Right">
                        ...
                        <Button DockPanel.Dock="Bottom" Name="Unassign" Padding="0" Margin="0,10,0,0" VerticalAlignment="Top" HorizontalContentAlignment="Stretch" Height="28" IsEnabled="False" >
                            <DockPanel  Margin="4,0" >
...
'@
        ...
        $WPFGui.SaveLogs.add_Click( {
                ...
                if ( $FileName ) {
                    ...
                    Save-FlowDocument -Document $WPFGui.Output.Document -Format $Format -FileName $FileName -Title "VMWare Tag Tool logs $(Get-Date -Format 'yyyy-MM-dd-HH-mm-ss')"
                }
            })
        ...
        $WPFGui.UpdateVMs.Add_Click( {

                # New Credential object from the form fields.
                if ( $WPFGui.Keys -notcontains 'Credential' ) {
                    $WPFGui.Add('Credential', (New-Object System.Management.Automation.PSCredential ($WPFGui.VCUsername.Text, $WPFGui.VCPassword.SecurePassword)))
                }
                $AsyncRunParameters = @{
                    Variables = @{
                        'WPFGui'     = $WPFGui
                    ...
                    Code      = {
                        $Error.clear()
                        # Disable controls and prep form fields to receive VM data
                        $WPFGui.UI.Dispatcher.invoke([action] {
                                $WPFGui.ControlGroup.IsEnabled = $false
                                $WPFGui.TagDescription.Text = ''
                                $WPFGui.TagList.Clear()
                                $WPFGui.TagSelector.IsEnabled = $false
                                $WPFGui.CategoryList.Clear()
                                $WPFGui.CategorySelector.IsEnabled = $false
                            })
                        ...
                        try {
                            if ( Connect-VIServerVerbose  -VIServer $VIServer -Credential $Credential) {
                                Get-VMData
                                $WPFGui.UI.Dispatcher.invoke([action] {
                                        $WPFGui.CategorySelector.IsEnabled = $true
                                    })
                                Disconnect-VIServerVerbose
                            }
                        }
        ...
        $WPFGui.VCUsername.add_TextChanged({ Sync-UpdateVMsButton })

        $WPFGui.VCPassword.add_PasswordChanged({ Sync-UpdateVMsButton })
            

        $WPFGui.CategorySelector.add_SelectionChanged( {
                # When a tag category is selected, add those tags to the TagList
                $WPFGui.ControlGroup.IsEnabled = $false
                $WPFGui.TagList.Clear()
                $WPFGui.CategorySelector.SelectedItem.Tags.Foreach({
                        $WPFGui.TagList.Add([PSCustomObject]@{
                                Name        = $_.Name
                                Description = $_.Description
                            })
                    })
                $WPFGui.TagSelector.IsEnabled = $true
                $WPFGui.ControlGroup.IsEnabled = $true
            })

        $WPFGui.TagSelector.add_SelectionChanged( {
                $WPFGui.Assign.IsEnabled = $true
                $WPFGui.Unassign.IsEnabled = $true
            })
        
        $WPFGui.Assign.Add_Click( {
                Update-TagAssignment -Assign
            })
        
        $WPFGui.Unassign.Add_Click( {
                Update-TagAssignment -Unassign
            })
        
        #endregion

        #region  and setup observable collections for controls
        $WPFGui.Add("VMList", (New-Object System.Collections.ObjectModel.ObservableCollection[Object]))
        $WPFGui.VMGrid.ItemsSource = $WPFGui.VMList
                
        $WPFGui.Add("TagList", (New-Object System.Collections.ObjectModel.ObservableCollection[PSCustomObject]))
        $WPFGui.TagSelector.ItemsSource = $WPFGui.TagList
                
        $WPFGui.Add("CategoryList", (New-Object System.Collections.ObjectModel.ObservableCollection[PSCustomObject]))
        $WPFGui.CategorySelector.ItemsSource = $WPFGui.CategoryList
                
        $WPFGui.Add('ConnectionState', 'Disconnected')
        #endregion
